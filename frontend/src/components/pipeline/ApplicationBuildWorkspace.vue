<template>
  <div class="page-stack build-workspace">
...
import ApplicationBuildHistory from './ApplicationBuildHistory.vue'
import ApplicationBuildDetail from './ApplicationBuildDetail.vue'
import QuickBuildDrawer from './QuickBuildDrawer.vue'
import {
  canRefreshHistory,
  createRequestGate,
...
const emit = defineEmits<{
  'application-loaded': [application: Application]
  'build-count': [count: number]
  'select-build': [buildId: number]
}>()
...
async function loadContext(selectLatest = false) {
  stopPolling()
  ...
  try {
    ...
    emit('application-loaded', applicationData)
    emit('build-count', buildItems.length)
    ...
    await resolveSelection(
      selectedBuild.value?.id === props.selectedBuildId,
      selectLatest ? undefined : props.selectedBuildId,
    )
  } catch (error) {
...
async function resolveSelection(preserveSelection = false, requestedId = props.selectedBuildId) {
  const result = resolveWorkspaceBuild(builds.value, requestedId)
  ...
  if (result.shouldSyncSelection) emit('select-build', result.build.id)
...
function selectBuild(buildId: number) {
  ...
  if (window.matchMedia('(max-width: 760px)').matches) void nextTick(() => detailPanel.value?.scrollIntoView())
}
...
async function refreshSelected() {
  ...
  try {
    ...
    if (!contextGate.isCurrent(contextGeneration) || selectedBuild.value?.id !== buildId) return
    builds.value = buildItems
    releaseBatches.value = batchItems
...
function openLatestBuild() {
  ...
  if (latest) emit('select-build', latest.id)
}
...
watch(() => props.selectedBuildId, buildId => {
  if (!loadingContext.value && builds.value.length) {
    if (buildId === selectedBuild.value?.id) return
    contextGate.next()
    void resolveSelection(false, buildId)
  }
})
