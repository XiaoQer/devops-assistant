...
- Project 的 CI/CD 工作台以紧凑 Application 卡片汇总最近构建、发布批次、环境目标和
  ...
  构建元数据及 Tekton 实际返回的 Task/Step 和单步骤日志；构建与各环境发布目标使用同一行紧凑
  执行标签和同一套步骤日志面板，切换标签后原位查看 Build 或 Deploy-only PipelineRun。等待审批或
  尚未创建 PipelineRun 的目标只展示真实业务状态。构建或任一环境目标处于活跃状态时自动刷新，
  刷新保留仍存在的环境选择，全部终态后停止轮询。桌面端历史构建栏固定在导航下方并在视口内
  独立滚动，避免右侧长日志在左列形成大块空白；移动端恢复普通上下布局。
- 发布历史、回滚流程和交付状态同步。
