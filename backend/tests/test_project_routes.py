...
from app.models import Application, ContainerRegistry, Project
...
class ProjectRoutesTest(unittest.TestCase):
    ...
    def test_member_and_cluster_routes(self):
        member_response = self.client.post(f"/api/projects/{self.project_id}/members", json={
            "name": "Bob",
            "email": "bob@example.com",
            "role": "admin",
        })
        cluster_response = self.client.post(f"/api/projects/{self.project_id}/clusters", json={
            "name": "prod-cn-1",
