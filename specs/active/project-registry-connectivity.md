...
- [ ] 给定完整的新 Registry 表单，当用户点击“测试连接”时，后端使用尚未保存的
      Server、用户名、Token 和 TLS 设置执行测试，并且不创建或修改数据库记录。
- [ ] 给定已保存 Registry，当用户在资源卡片点击“测试连接”时，后端使用解密后的
      Token 测试连接，并保存 `connected` 或 `failed`、最近测试时间和脱敏消息。
- [ ] 给定编辑中的 Registry 且 Token 留空，当用户测试或保存时，后端沿用已保存 Token，
      API 不向前端回显 Token。
- [ ] 给定 Registry 返回 Basic 认证挑战，当凭据有效时测试成功；凭据无效时返回
      `authentication_failed`。
- [ ] 给定 Registry 返回 Bearer Challenge，当授权端点接受凭据并签发 Token 时，后端用
      Bearer Token 再次请求 `/v2/` 并确认成功。
- [ ] 给定 ACR、Harbor、Docker Hub、ECR、GCR、Generic OCI 或 GHCR，当配置合法时
      Provider 校验接受该类型，并使用统一 OCI 流程测试。
- [ ] 给定未填写用户名或 Token 的新配置，当测试或保存时，后端拒绝请求，不执行匿名测试。
- [ ] 给定连接测试失败，当用户保存配置时，保存仍可完成，新配置状态为 `untested`，
      失败结果不会被客户端伪装成已持久化的成功状态。
- [ ] 给定 Server、用户名、Token 或 TLS 设置发生变化，当保存后，历史连接状态、时间和
      消息被重置；只修改名称或 Namespace 不重置状态。
- [ ] 给定有效证书，当正常测试成功时，结果包含 `tls_verified: true`。
- [ ] 给定无效或自签名证书且未跳过校验，当测试时返回 `tls_failed`；开启跳过校验后允许
      继续认证，但结果包含 `tls_verified: false`，页面持续显示风险警告。
- [ ] 给定 HTTP 地址、loopback、link-local、云元数据地址或其他明确危险目标，当测试时
      后端拒绝连接；企业私网 Registry 地址仍允许测试。
- [ ] 给定跨 Project 的 Registry ID，当读取、修改、删除、设为默认或测试时，接口返回
      资源不存在，不泄露另一 Project 的 Registry 信息。
- [ ] API、日志、异常、测试数据和响应中不出现明文 Token、Authorization Header、Bearer
      Token、加密凭据或未脱敏的远端响应正文。
- [ ] 页面卡片展示未测试、已连接或连接失败、最近测试时间、脱敏消息以及 TLS 风险状态。
- [ ] 所有新增接口保持 `success`、`message`、`data`、`timestamp`、`trace_id` 响应结构。
- [ ] 后端相关自动化测试、前端类型检查和生产构建通过，`./scripts/verify.sh` 成功。
- [ ] `docs/current-state.md` 只在实现和验证完成后记录 Registry 连通性能力。
...
实现过程中补充测试命令、结果和必要的人工页面检查。当前仅完成需求与设计确认，尚未开始
实现或验证。

## 完成

实现满足全部验收条件并经用户验收后，将状态改为“已验收”，记录日期，并把本文件移至
`specs/completed/`。
