...
- [x] 给定用户进入 Project 的 Kubernetes 页面，当点击“添加集群”时，则同时看到“初始化
  新 Kubernetes 集群”和“添加已有集群”入口，且前者明确不可用并说明依赖 Aliyun 能力。
- [x] 给定合法 kubeconfig 包含多个 contexts，当用户粘贴配置时，则默认选中
  `current-context`，并可选择其他 context。
- [x] 给定内置或自定义环境标签，当用户保存已有集群时，则标签随集群持久化并在列表展示。
- [x] 给定合法 kubeconfig，当用户保存集群时，则数据库只保存 Fernet 密文，序列化只返回
  `has_kubeconfig`，不返回 kubeconfig 明文或密文。
- [x] 给定编辑已有集群，当页面打开编辑表单时，则 kubeconfig 不回显；留空保存会保留原凭据，
  提交新 kubeconfig 会替换凭据并把连接状态重置为 `untested`。
- [x] 给定尚未保存的合法配置，当用户点击“测试连接”时，则系统使用表单 kubeconfig 和所选
  context 请求 Kubernetes Version API，并返回连接状态、API Server 和 Kubernetes 版本，
  不写入集群记录。
- [x] 给定已保存集群，当用户点击“测试连接”时，则系统解密该集群的 kubeconfig 完成同样测试，
  并更新 `connection_status`、`last_checked_at`、`kubernetes_version`；成功时更新实际
  `api_server`。
- [x] 给定网络、认证或证书错误，当执行连通性测试时，则接口返回 `connected: false` 和脱敏的
  分类消息，不要求测试成功才允许保存，也不长期保存失败详情。
- [x] 给定包含 `exec` 认证、本地证书/密钥/Token/CA 文件引用、非 HTTPS Server、缺失所选
  context 或超过 1 MiB 的 kubeconfig，当测试或保存时，则请求以明确的 400 错误被拒绝。
- [x] 给定其他 Project 的 cluster id，当请求重测时，则返回项目作用域内的不存在响应，不能
  使用或泄露该集群配置。
- [x] 给定迁移前已有集群，当升级数据库后，则记录保留，环境标签可为空、状态为 `untested`、
  `has_kubeconfig` 为 false，并可通过编辑补充新字段。
- [x] 所有新增 API 继续使用 `success`、`message`、`data`、`timestamp`、`trace_id` 统一结构。
- [x] 相关迁移、Service 和路由自动化检查通过，前端类型检查、生产构建和
  `./scripts/verify.sh` 通过。
- [x] `docs/current-state.md` 只在验证完成后记录该能力已实现，并保留 Aliyun 初始化尚未实现
  的边界。
...
- 2026-07-13：`backend/.venv/bin/python -m pytest tests/test_kubernetes_cluster_migration.py -q`
  通过，2 个迁移与序列化测试成功。
- 2026-07-13：`backend/.venv/bin/python -m pytest tests/test_kubernetes_cluster_service.py -q`
  通过，8 个 kubeconfig 安全、加密和连通性测试成功。
- 2026-07-13：`backend/.venv/bin/python -m pytest tests/test_project_routes.py
  tests/test_validation_routes.py -q` 通过，22 个项目路由与输入校验测试成功。
- 2026-07-13：`npm --prefix frontend run build` 通过 TypeScript 检查和 Vite 生产构建。
- 2026-07-13：`./scripts/verify.sh` 通过，后端 98 个测试全部成功，前端生产构建成功。
- 2026-07-13：敏感字段边界检查确认模型响应不包含 `encrypted_kubeconfig` 或 kubeconfig
  明文，测试凭据只存在于自动化测试夹具。

## 完成

本规格已于 2026-07-13 验收并归档；Aliyun ACK 初始化和 Application Environment 的
多集群运行客户端切换仍属于后续能力。
