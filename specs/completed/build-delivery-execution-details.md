...
- [x] Build Pipeline 按 API 返回顺序展示真实 Task/Step，不再固定为 Clone/Build/Push。
- [x] Java、Node 和 Dockerfile 构建的实际步骤名称和日志均被保留。
- [x] 所选构建关联的全部发布环境均按 target 独立展示，追加环境刷新后出现。
- [x] 等待审批和没有 PipelineRun 的环境展示真实业务状态，不生成执行步骤。
- [x] 选择已有 PipelineRun 的环境后展示实际 Deploy Task/Step 和单步骤日志。
- [x] 切换构建或环境时，旧日志响应不能覆盖当前选择。
- [x] 默认选择第一个失败步骤，其次运行步骤，否则第一个步骤。
- [x] 构建或任一环境处于活跃状态时自动刷新，全部终态后停止轮询。
- [x] 部分环境失败不改变其他环境的展示状态。
...
- `frontend/src/features/build-explorer/state.test.ts` 覆盖 Java、Node、Dockerfile 实际步骤保序、
  构建与发布批次关联、默认步骤/环境选择、审批等待态及活跃交付轮询。
- 2026-07-15 运行 `./scripts/verify.sh`：后端 210 项测试通过，前端 19 项测试通过，Vue 类型检查
  与 Vite 生产构建通过。
