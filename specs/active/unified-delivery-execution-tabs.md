...
- [ ] “构建”与发布环境按同级标签展示，构建固定在首位。
- [ ] 页面只显示一套步骤选择和日志面板，不再上下重复。
- [ ] 默认选中构建；选择环境后原位展示其 Deploy-only Pipeline 实际步骤和日志。
- [ ] 等待审批或没有 PipelineRun 的环境显示真实状态且不请求日志。
- [ ] 切换构建时回到构建标签，切换环境时旧日志不能覆盖新选择。
- [ ] 轮询刷新后尽量保持当前环境选择，target 不存在时回到构建。
- [ ] 没有环境发布时不展示多余空区块。
- [ ] 窄屏执行标签可横向滚动，日志区域不发生额外上下跳动。
